...
class RuntimeScenarioTests(unittest.TestCase):
    ...
    def test_multi_day_docker_failure_recovery_and_interruption(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.record(root, "operation_started", "2026-08-28T14:40:00Z", "Run integration tests", "--run-id", "run-1", "--operation-id", "integration-tests", "--attempt", "1", "--category", "verification")
            ...
            self.record(root, "run_resumed", "2026-08-29T09:00:00Z", "Verification resumed", "--run-id", "run-2", "--resumes-run-id", "run-1", "--lifecycle", "VERIFICATION")
            self.record(root, "operation_started", "2026-08-29T09:05:00Z", "Run final verification", "--run-id", "run-2", "--operation-id", "final-verification", "--attempt", "1", "--category", "verification")
            self.record(root, "operation_succeeded", "2026-08-29T09:35:00Z", "Final verification passed", "--run-id", "run-2", "--operation-id", "final-verification", "--attempt", "1", "--category", "verification")
            self.record(root, "run_finished", "2026-08-29T10:00:00Z", "Verification run finished", "--run-id", "run-2", "--lifecycle", "VERIFICATION")
